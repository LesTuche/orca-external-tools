...
def cli_parse(args: list[str], mode: RunMode) -> Namespace:
    ...
    ).parent / 'models'  # .dirname(os.path.abspath(__file__))
    ...
    if mode in (RunMode.Standalone, RunMode.Server):
        parser.add_argument(
            ...
            type=str,
            default="omol",
            help='The MACE model file name (must be in MODEL_DIR) or absolute path. Default: "omol".')
        parser.add_argument(
            "-d", "--model-dir",
            metavar="MODEL_DIR",
            type=Path,
            default=default_model_path,
            help=f'The directory to look for MACE model files. Default: "{default_model_path}".')
    ...
    if mode is RunMode.Server:
        ...
        if not is_port_available(host, port):
            print(
                f"Port {port} on {host} is already in use. Selecting a free one...")
            port = get_free_port()
