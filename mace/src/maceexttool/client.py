...
def submit_mace(server_url: str,
    ...
    """
    Sends a MACE calculation to the server and returns the result.
    ...
    try:
        response = requests.post(
            'http://' + server_url + "/calculate", json=payload)
...
def run(arglist: list[str]):
    """Run a calculation on a given structure on the MACE server."""
    ...
    # submit to the mace calculator server
    energy, gradient = submit_mace(server_url=args.bind, atom_types=atom_types, coordinates=coordinates,
    ...
    # Timings
    print("Total time:  {:6.3f} seconds".format(
